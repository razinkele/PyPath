...
def test_app_ui_uses_shinyswatch_theme_when_available():
    ...
    import pypath_shiny.app as app_mod
...
def test_app_imports_without_shinyswatch(monkeypatch, tmp_path):
    ...
    try:
        # Ensure importing pypath_shiny.app works when shinyswatch not installed
        if "pypath_shiny.app" in sys.modules:
            del sys.modules["pypath_shiny.app"]
        # Import freshly
        import importlib

        app_mod = importlib.import_module("pypath_shiny.app")
