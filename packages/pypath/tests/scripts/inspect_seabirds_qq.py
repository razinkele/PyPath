...
params_dict = {
    ...
    "PP_type": getattr(
        pypath_ecosim.params, "PP_type", np.zeros(pypath_ecosim.params.NUM_GROUPS + 1)
    ),
...
k1 = deriv_vector(
    state.copy(),
    params_dict,
    {"Ftime": np.ones_like(state)},
    {"FishingMort": np.zeros_like(state)},
)
state_k2 = state + 0.5 * dt * k1
k2 = deriv_vector(
    state_k2,
    params_dict,
    {"Ftime": np.ones_like(state_k2)},
    {"FishingMort": np.zeros_like(state_k2)},
)
state_k3 = state + 0.5 * dt * k2
k3 = deriv_vector(
    state_k3,
    params_dict,
    {"Ftime": np.ones_like(state_k3)},
    {"FishingMort": np.zeros_like(state_k3)},
)
state_k4 = state + dt * k3
k4 = deriv_vector(
    state_k4,
    params_dict,
    {"Ftime": np.ones_like(state_k4)},
    {"FishingMort": np.zeros_like(state_k4)},
)

for label, st in [("k1", state), ("k2", state_k2), ("k3", state_k3), ("k4", state_k4)]:
    QQs = _compute_Q_matrix(params_dict, st, {"Ftime": np.ones_like(st)})
    print(
        f"Stage {label}: total consumption by Seabirds =",
        float(np.nansum(QQs[:, sidx])),
    )
