...
@pytest.mark.xfail(
    reason="R reference diagnostics generated by buggy ensure_mat (reshapes 1-D link arrays "
    "into n×n matrices with byrow recycling instead of PreyFrom/PreyTo mapping), producing "
    "~1000x inflated QQ/consumption values.  Regenerate reference data after fixing "
    "scripts/extract_rpath_data.R ensure_mat().",
    strict=False,
)
def test_seabirds_termwise_matches_rpath_first_12_months():
    ...
    params_dict = {
        ...
        "PP_type": getattr(
            pypath_ecosim.params,
            "PP_type",
            np.zeros(pypath_ecosim.params.NUM_GROUPS + 1),
        ),
    ...
    fishing_dict = _normalize_fishing_input(
        pypath_ecosim.fishing, pypath_ecosim.params.NUM_GROUPS + 1
    )
    ...
    for m in range(1, months + 1):
        ...
        no_integrate_mask = (
            np.asarray(
                params_dict.get("NoIntegrate", np.zeros(params_dict["NUM_GROUPS"] + 1))
            )
            != 0
        )
        ...
        QQ = _compute_Q_matrix(
            params_dict, state_for_Q, {"Ftime": np.ones_like(state_for_Q)}
        )
        ...
        # fishing mort
        fishing_mort = fishing_dict.get(
            "FishingMort", np.zeros(pypath_ecosim.params.NUM_GROUPS + 1)
        ).copy()
