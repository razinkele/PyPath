...
def test_m0_unchanged_when_fishing_correct():
    ...
    forcing0 = {
        ...
        "ForcedBio": np.where(
            scenario.forcing.ForcedBio[0] > 0, scenario.forcing.ForcedBio[0], 0
        ),
        "ForcedMigrate": scenario.forcing.ForcedMigrate[0],
        "ForcedEffort": (
            scenario.fishing.ForcedEffort[0]
            if 0 < len(scenario.fishing.ForcedEffort)
            else np.ones(rs.NUM_GEARS + 1)
        ),
    }
