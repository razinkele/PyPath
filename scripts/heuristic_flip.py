"""
Heuristic flip simulation: try flipping each eligible EE>1 candidate (cap EE=1 and set biomass=NaN),
...
# Identify candidates: groups with ee>1 in base_diag and eligible (original_no_ee & original_no_b)
candidates = []
for idx in living_idx:
    if original_no_ee[idx] and original_no_b[idx]:
        if base_model.EE[idx] > 1.0:
            candidates.append(idx)

print('Candidates for flipping:', candidates)

if not candidates:
    print('No eligible candidates found. Exiting.')
    raise SystemExit(0)
...
for cand in candidates:
    ...
    results.append((cand, score, score_A, score_b))
    print(f'Candidate {cand}: score={score:.4f} (A={score_A:.4f}, b={score_b:.4f})')
    if score < best_score:
        best = cand
        best_score = score
...
print(f'Best flip: {best} with score {best_score}')
...
out = {
    ...
    'results': [{'cand': int(c), 'score': float(s), 'A': float(a), 'b': float(b)} for c, s, a, b in results]
}
json.dump(out, open(ECOPATH_DIR / 'heuristic_flip_results.json', 'w'), indent=2)

# Save final diagnostics to file
np.save(ECOPATH_DIR / 'heuristic_final_A.npy', final_diag['A'])
np.save(ECOPATH_DIR / 'heuristic_final_bvec.npy', final_diag['b_vec'])

print('Applied flip and saved final diagnostics.')
