...
pred_idx = seabirds_idx + 1  # 1-based indexing used in ecosim arrays
...
month1_comparison = []
for prey in range(1, sp.NUM_GROUPS + 1):
    qcalc = q_month1[prey]
...
for month in range(1, 1200 + 1):
    ...
    predation_loss = sum(qvals.values())
    consumption_by_pred = 0.0
    # consumption by seabirds is sum over prey QQ[:,pred_idx]
    consumption_by_pred = sum([compute_qcalc_for_pred(state, forcing, p,)[pred_idx] if False else 0.0])
    # Because compute_qcalc_for_pred gives Q for given predator; consumption_by_pred is sum QQ[:, pred]
    ...
    predYY = np.zeros(NUM_GROUPS + 1)
    for j in range(1, sp.NUM_LIVING + 1):
        if Bbase[j] > 0:
    ...
    # M0 loss
    M0_arr = sp.MzeroMort
    m0_loss = float(M0_arr[pred_idx] * BB[pred_idx]) if len(M0_arr) > pred_idx else 0.0
...
with open(DIAG_DIR / 'seabirds_root_cause_summary.json', 'w') as fh:
